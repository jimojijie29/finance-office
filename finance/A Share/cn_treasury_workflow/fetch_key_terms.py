...
使用方法:
    python fetch_key_terms.py [年月]
    python fetch_key_terms.py all
"""
...
CONFIG = {
    'data_file': 'cn_treasury_key_terms.csv',
    'ts_code': '1001.CB',
    'curve_type': '0',
    'page_size': 2000,
}
...
def process_data(raw_df):
    ...
    # 只保留关键期限
    raw_df = raw_df[raw_df['curve_term'].isin(TERM_MAPPING.keys())]
    ...
    # 只保留有完整数据的日期（三个期限都有）
    df_result = df_result.dropna(subset=TARGET_COLS, how='any')
    
    log(f"  处理后: {len(df_result)}个交易日（完整数据）")
...
def main():
    print("="*60)
    print("中国国债收益率数据 - 关键期限版 (1年/5年/10年)")
    print("="*60)
    
    if len(sys.argv) < 2:
        print("\n使用方法:")
        print("  python fetch_key_terms.py [年月]  # 如 202301")
        print("  python fetch_key_terms.py all     # 获取所有缺失月份")
        return
