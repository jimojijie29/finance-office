...
CONFIG = {
    # 数据文件路径
    'data_file': 'us_treasury_yields_all_terms.csv',
    
    # mx-macro-data 技能路径
    'mx_macro_skill_path': r'D:\OpenClawData\.openclaw\workspace\skills\mx-macro-data',
    
    # R 可视化脚本路径
    'r_script': 'us_treasury_workflow_v2.R',
    
    # 输出 HTML 文件
    'output_html': 'us_treasury_all_terms.html',
...
def fix_html_encoding(working_dir):
    """修复HTML文件的编码问题（Windows中文乱码）"""
    html_file = os.path.join(working_dir, CONFIG['output_html'])
    
    if not os.path.exists(html_file):
        return
    
    try:
        # 在Windows上，R生成的HTML可能是GBK编码，需要转换为UTF-8
        # 首先尝试用GBK读取
        content = None
        try:
        ...
        if content is None:
            log("警告: 无法读取HTML文件进行编码修复")
            return
        
        # 检查是否包含中文按钮标签（如果已经是正确的UTF-8，就不需要修复）
        if '全部' in content and '超短期' in content:
            log("HTML编码正常，无需修复")
            return
...
def generate_visualization():
    ...
    try:
        ...
        # 修复HTML编码问题（Windows中文乱码）
        fix_html_encoding(working_dir)
