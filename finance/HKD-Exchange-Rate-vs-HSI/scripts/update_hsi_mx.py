...
def fetch_hsi_with_skill():
    ...
    log("调用 mx-macro-data skill 获取数据...")
...
def parse_mx_data(csv_path):
    ...
    try:
        ...
        # 查找恒生指数行（使用原始数值，不是"万"单位）
        hsi_row = None
        for idx, row in df.iterrows():
            indicator = str(row.get('indicator_name', ''))
            # 选择不包含"万"的那一行（原始数值）
            if '恒生指数' in indicator and '(点)' not in indicator and '万' not in indicator:
                hsi_row = row
                break
        
        # 如果没找到，使用第二行（通常是原始数值行）
        if hsi_row is None and len(df) >= 2:
            hsi_row = df.iloc[1]
        elif hsi_row is None:
            hsi_row = df.iloc[0]
...
def update_hsi():
    """更新恒生指数数据"""
    log("="*60)
    log("更新恒生指数数据 (mx-macro-data skill)")
    log("="*60)
