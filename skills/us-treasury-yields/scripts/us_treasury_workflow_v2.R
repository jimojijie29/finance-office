...
# ============================================
df <- read.csv("data/us_treasury_yields_all_terms.csv", fileEncoding = "UTF-8")
...
htmlwidgets::saveWidget(fig, "data/us_treasury_all_terms.html", selfcontained = TRUE)

# Windows下需要额外处理编码问题
if (.Platform$OS.type == "windows") {
  # 读取生成的HTML文件
  html_content <- readLines("data/us_treasury_all_terms.html", encoding = "UTF-8", warn = FALSE)
  # 确保以UTF-8编码写回
  writeLines(html_content, "data/us_treasury_all_terms.html", useBytes = TRUE)
}

# 使用英文输出避免编码问题
cat("\n[OK] US Treasury Yield Curve Chart Generated Successfully!\n")
cat("Output file: data/us_treasury_all_terms.html\n")
